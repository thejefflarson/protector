...
/// problem the drop counter below measures). LRU so a churn of distinct destinations can't
...
/// in-kernel keeps the (very high) file-open volume off the ring buffer. This filter widens
/// past tmpfs for a small, fixed allowlist of on-host credential-file basenames (see
