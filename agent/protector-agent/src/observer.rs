...
mod ebpf {
    ...
    enum RawEvent {
        ...
        /// Ptrace ATTACH access check: a process attempted to PTRACE_ATTACH (or
        ...
        /// Kernel module load: `init_module`/`finit_module` reached
    ...
    impl EbpfObserver {
        pub async fn run(
            ...
            // cumulative; the heartbeat snapshots them to surface the numbers 's
            ...
            let result = loop {
                tokio::select! {
                    ...
                    _ = heartbeat.tick() => {
                        ...
                        // emit the per-node numbers needs measurable —
        ...
        async fn attribution_worker(
            ...
            tracing::info!(pods = table.len(), "cgroup attribution table built");
            ...
            loop {
                ...
                let uid = match Self::resolve(&table, &mut fallback_cache, attr) {
                    ...
                    PodAttribution::Unreadable => {
                        ...
                        // 's "low unresolved attribution" measures per node.
        ...
        fn decode(data: &[u8]) -> Option<RawEvent> {
            ...
            match header.kind {
                ...
                // both bodies ARE the header — already parsed above, and its length
...
mod rate_tests {
    ...
    fn zero_signals_is_zero_rate() {
        ...
        // "no drops / no traffic" datapoint needs.
