...
// poll is modelled by re-rendering with a new `view` prop.
...
describe("Alerts content-hash reconcile keying", () => {
  it("keeps an identical persisting alarm as the SAME node (no flicker) across a poll", () => {
    ...
    const { container, rerender } = render(<AlertsView view={alertsView([a])} />);
    ...
    rerender(<AlertsView view={alertsView([alert({ signal: "notable exec: bash", workload: "web" })])} />);
  ...
  it("renders a genuinely new alarm as a NEW node", () => {
    const { container, rerender } = render(<AlertsView view={alertsView([alert({ signal: "notable exec: bash" })])} />);
    ...
    rerender(
      <AlertsView
        view={alertsView([alert({ signal: "notable exec: bash" }), alert({ signal: "contacted cloud-metadata" })])}
      />,
    );
