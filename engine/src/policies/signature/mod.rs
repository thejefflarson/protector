...
pub struct SignaturePolicy {
    ...
    /// policy behaves EXACTLY as the pre- gated gate (unconfigured = byte-identical shadow).
...
impl SignaturePolicy {
    ...
    /// the minimal constructor the existing tests + the pre- gated deploy use unchanged.
    ...
    /// The legacy gated single-identity gate's decision for `req` (the pre- behavior,
