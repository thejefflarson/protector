...
pub(super) fn scope(enforce: bool) -> EnforceScope {
    if enforce {
...
pub(super) fn pod_request(images: &[&str]) -> AdmissionRequest<DynamicObject> {
...
pub(super) struct FakeChecker(pub(super) HashMap<String, bool>);
...
pub(super) fn policy(verdicts: &[(&str, bool)], enforce: bool) -> SignaturePolicy {
...
/// repeated verification across the enforce + shadow paths ('s zero-egress constraint).
...
pub(super) struct FakeObserver {
    postures: HashMap<String, SigningPosture>,
    calls: Arc<AtomicUsize>,
}

impl FakeObserver {
    pub(super) fn new(postures: Vec<(&str, SigningPosture)>) -> Self {
        Self {
            postures: postures
                .into_iter()
...
pub(super) fn signer(identity: &str, issuer: &str) -> Signer {
    Signer {
