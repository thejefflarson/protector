...
/// Fixed mount paths for the exploitation-intel feeds. owns the feed *mechanism*
...
async fn run() -> Result<()> {
    ...
    // The bounded, deduped admission-decision ring: the webhook engine writes
    ...
    // The durable decision journal: the webhook engine persists each resolved
    ...
    let mut engine_task: Option<tokio::task::JoinHandle<()>> = None;
    {
        ...
        // defaults to the fixed feeds mount (owns the feed mechanism); the env is a
