...
//!
//! recorded ONLY violations (audit/deny), so a healthy cluster's admission-decision
//! log was empty and read as broken. records EVERY resolved admission — including clean
...
//!
//! Volume is the reason stayed violations-only: a Deployment's N replicas and a
//! CronJob's repeated runs would flood an admit-everything log. bounds it two ways:
...
/// every field that post-dates uses `#[serde(default)]` so an older journal line still
...
pub struct PolicyDecisionRecord {
    ...
    /// Defaults to `true` so an older journal line predating this field (no field) reads as
    /// "no would-deny known".
    #[serde(default = "yes")]
...
/// `serde` default for `would_admit` on lines that predate this field: a pre-existing
/// journal line has no counterfactual, so default to "would admit" (no known would-deny).
fn yes() -> bool {
    true
}
...
mod tests {
    ...
    fn replica_churn_dedups_into_a_single_counted_row() {
        // The whole point of 's bounding: a Deployment's N replicas (same subject +
    ...
    fn record_round_trips_through_json_with_back_compat_defaults() {
        ...
        assert!(
            parsed.would_admit,
            "a line predating the field, with no would_admit, defaults to true"
        );
    }
}
