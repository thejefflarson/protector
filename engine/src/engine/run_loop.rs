...
pub async fn run_watch(
    ...
    // (`PROTECTOR_MCP_AUDIT_PATH`); unset/unwritable ⇒ in-memory only (no crash), and the "Access"
    // tab then shows the honest "resets on restart" caveat.
    let mcp_audit = std::sync::Arc::new(crate::engine::mcp::AccessAuditSink::from_env());
