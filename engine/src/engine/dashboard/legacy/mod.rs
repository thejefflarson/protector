...
//!
//! ADR-0019 adopts `maud` and a React-like `view_model` / `components` split. The nav and
//! status banner are migrated as the proof-of-pattern (JEF-204), and the diagnostics panels
//! (readiness, first-run, bake, reversions, attack-vectors) in JEF-206; the remaining
//! pieces (findings cards, report, judgements) still render via the old `format!`-string
//! helpers and live here until their tickets migrate them onto the maud components. This
//! module exists ONLY to keep each file under the repo's 1,000-line cap while that migration
//! is in flight — every helper here is slated for deletion. The `Readiness` data type and
//! `derive_readiness` stay (the DATA layer the panel view-model + `/readiness` JSON read).
//!
