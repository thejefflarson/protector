//! Page-shell tests for the ROOT-ONLY dashboard document (superseding ADR-0025's
...
fn the_body_is_root_only_no_server_strip_or_nav() {
    // the LAST server-rendered body parts (the status strip + the tab nav) moved to the
...
fn the_shell_is_flag_free_and_carries_no_maud_body() {
    // The per-tab flag is gone: no `data-preact-tabs` list — every tab is client-rendered.
