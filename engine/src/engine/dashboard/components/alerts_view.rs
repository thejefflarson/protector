//! The Alerts view body (JEF-323): the live "alarming-now" corroboration list — each event's
//! signal, the (informer-resolved) workload, recency, and the objective/chain it corroborates.
//! A CURRENT-WINDOW view of what is alarming THIS pass (labelled honestly as live, not a scrolling
//! audit log). An alert is EVIDENCE (corroboration), NEVER a verdict — the copy never implies a
//! breach conclusion or that any action was taken (default posture is shadow/propose, ADR-0016).
//! The empty state is CALM ("no alarming activity right now"), unless a node is blind, in which
//! case the reassurance is replaced by the "absence is not safety" caveat (JEF-308). Pure
...
/// the workload it was attributed to, its recency, and the chain it corroborates (if any). Every
/// untrusted string (signal / workload / chain) is auto-escaped by maud (`{}`, never `PreEscaped`).
    ...
    html! {
        li class={ "alert-card alert-" (a.kind) } {
            ...
            div.alert-meta.muted {
                ...
                @match &a.corroborates {
                    Some(chain) => span.alert-corroborates {
                        span.alert-label { "corroborates " }
                        (chain)
                    }
                    None => span.alert-corroborates.muted {
...
fn empty_state(v: &AlertsViewProps) -> Markup {
    ...
    html! {
        div.empty.empty-alerts-calm {
            ...
            p.empty-sub.muted {
                "no runtime signal is alarming this pass \u{2014} nothing is currently corroborating \
                 an attack. This is a live window, not a history."
            }
        }
    }
}
