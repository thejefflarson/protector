...
//!      reveals alone. It is surfaced as a **divergence finding through the continuity-drift regression
...
/// Encode a divergence finding as a `SigningRegression/<repo>` row so it rides the continuity-drift
/// regression channel (the inventory partitions it out of the decision tallies and renders the loud banner).
