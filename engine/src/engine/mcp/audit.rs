//! The audit-emission seam (ADR-0031 §4). Every MCP response ABOVE the safe-by-construction
...
//! SEPARATE ticket; it wires a durable implementation of this same trait in — nothing in
...
/// as seconds since the Unix epoch so the record is self-contained (a durable sink can
...
/// The pluggable audit seam a durable sink is wired into. A `redacted`-tier response never calls this
...
/// observability pipeline until a future sink attaches a durable store + the "Access" tab.
