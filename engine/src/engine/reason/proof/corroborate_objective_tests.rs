...
fn api_server_peer_corroborates_foothold() {
    // This fixture resolves the apiserver ClusterIP to the `default/kubernetes` label.
...
/// (the ADR-0011 false positive). Corroboration promotes notable execs only, not privesc.
#[test]
...
/// the gap this issue closes: a `LibraryLoaded` (vuln-matched by) on an
    ...
    let snap = Snapshot {
        ...
        image_vulns: vec![ImageVulnerabilities {
            ...
            vulnerabilities: vec![Vulnerability {
                ...
                exploited_in_wild: true,
                epss: None,
                // The loaded library below must match a CVE package so the pkg-name
                // normalizer keeps it (`libssl.so.1.1` and `openssl` both normalize to `ssl`).
