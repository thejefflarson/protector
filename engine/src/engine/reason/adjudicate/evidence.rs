...
fn cve_evidence_budgeted(v: &Vulnerability, budget: &mut usize) -> String {
    // Fix availability is the exploitability signal is after: a fix existing
...
/// order), then collapse duplicate CVE ids to one representative (source of truth, so
...
/// `budget` — the loaded-at-runtime-only counterpart of [`entry_evidence_budgeted`],
...
///   (including CLUSTER connections, whose existing resolution is untouched) renders via
...
/// `budget` — the behavior-line counterpart of [`entry_evidence_budgeted`] ('s
