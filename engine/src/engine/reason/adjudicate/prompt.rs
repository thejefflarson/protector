...
/// downstream nodes" precedent those same builders already document for) — only the
...
fn build_delta_prompt_inner(
    ...
    // true EXACT-STATE guard (an identical full state always HITS, restart-safe with) and
...
fn render_evidence(
    ...
    // — both per-field and against a per-entry aggregate budget (in `entry_evidence`)
    ...
    cves.sort();
    cves.dedup();
    //  (skip non-reachable CVEs, see `evidence::retain_reachable_cves`): the judge
