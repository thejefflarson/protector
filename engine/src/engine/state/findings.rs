...
/// notifies EXACTLY ONCE per collapse and once per recovery, never per pass. Reuses 's stall
...
/// list — the named node, its FIXED mechanism string (never model/untrusted text,
...
/// cut-set list. `None` on [`Finding`] while the entry hasn't been through the
...
pub struct IncidentSummary {
    ...
    /// would-act aggregation classifies on: never re-derived from verdict prose.
...
impl IncidentSummary {
    ...
    /// display-only [`IncidentSummary`] the finding detail renders. Recomputes each
...
pub struct Finding {
    ...
    /// cut-set list. `None` when the entry hasn't been through the incident
...
impl Finding {
    ...
    /// cut-set list, specifically each chosen cut's advisory blast-radius note.
    pub fn from_chain(
        chain: &ProvenChain,
        graph: &SecurityGraph,
        ...
        Finding {
            ...
            // — projected into the display-only shape the finding detail renders.
            incident: decisions
                .get(&chain.entry.0)
...
impl Findings {
    ...
    /// `decisions` + `health` feed each finding's model-chosen cut-set — see
    /// [`Finding::from_chain`].
