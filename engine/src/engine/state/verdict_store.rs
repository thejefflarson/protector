...
/// single-slot behaviour that thrashes on an A→B→A flip, so even a hostile/typo'd env can't
...
impl BakeStats {
    ...
    /// This is the engine-side resolution rate the behavioral-bake reads attribution quality from.
    #[allow(dead_code)]
...
pub struct VerdictEntry {
    ...
    /// the re-judge gate (formerly `verdict_cache`, a single slot), widened from
...
impl VerdictStore {
    ...
    ///   (the backoff's Uncertain-never-cached discipline lives on the cache path; this is its
