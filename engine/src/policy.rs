...
pub struct Engine {
    ...
    /// audit, or deny — is mirrored here (recording the good pods too, not only
    /// violations).
...
/// row recorded (one row per request, deduped, not one per policy).
///
/// This summary type changed the signature/mesh statuses from a COARSE two-state ("passed the gate" vs
...
impl Engine {
    ...
    /// Attach the admission-decision ring (237) so EVERY resolved decision — clean
...
mod tests {
    ...
    async fn records_clean_admit_as_an_allow_row() {
        // The headline behavior: a good pod (signed + meshed) is RECORDED as a green admit row,
    ...
    async fn replica_churn_dedups_into_one_counted_admission_row() {
        ...
        // counted row, not flood the ring — exactly what the dedup+ring-cap bounding requires.
        let log = Arc::new(PolicyDecisionLog::new());
        let engine = Engine::new(
            vec![Box::new(Fixed {
                name: "image-signature",
