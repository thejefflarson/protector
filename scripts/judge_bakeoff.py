...
# ADR-0023 "Changes since the last decisive verdict" delta block), and `downstream` (the
...
CASES = [
    ...
    # A real-world false-breach fixture (argocd-server, v0.3.100). An
...
DEFAULT_MODELS = [
    ...
    "qwen3:1.7b",                            # LEAD CANDIDATE (ADR-0026) — 12/12 here (only model to sweep all three evidence types + every refute), standard transformer (KV cache works)
...
def bench(models):
    ...
    for m in models:
        ...
        for case in CASES:
            ...
            downstream = case[9] if len(case) > 9 else "(none)"  # optional downstream-evidence block
