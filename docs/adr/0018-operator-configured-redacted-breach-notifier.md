...
([`journal.rs`](../../engine/src/engine/journal.rs))
...
operator most needs a push, the notifier stays silent. This extension adds a second, narrow
...
- **Edge-triggered, not per-pass.** Fires exactly once when a was-covering fleet goes
  ...
  when it recovers (`runtime_coverage_restored`) — reusing the stall debounce's hysteresis so a
