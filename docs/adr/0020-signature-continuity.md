...
2. **Signing downgrade is a first-class regression class.** An established **keyless**
   ...
   that same admission-finding path (audit-only, shadow; ADR-0016) and feeds the honesty
...
3. **Provenance change is a drift class on the same audit channel.** A verified
...
1. **would-admit is the negation of a REGRESSION, not a posture test.** The counterfactual
   ...
   from the baseline-relative drift verdict (280), NOT the raw posture:
...
1. **Deny on a REGRESSION, in enforced scope only, and nowhere else.** The admission webhook's
   ...
   **established** baseline is a `Deny`; out of scope it is an `Audit` (recorded, still admitted).
   ...
   the inventory's presentation `SigningEnforcement::WouldBlock` projects, so the inventory's "would block"
...
2. **Cold-start never denies.** A freshly-learned / not-yet-`established` baseline (its 24h
