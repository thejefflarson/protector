...
free (it busts once on real change, not per pass — the model-call budget).
...
implementation of the revert lives in the
[`ReversionLog`](../../engine/src/engine/state/reversion.rs); this ADR is the decision
record only.
